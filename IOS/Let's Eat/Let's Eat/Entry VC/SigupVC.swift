   ...
   // @IBOutlet weak var textConfirmEmail: UITextField!
    ...
    @IBAction func signupTapped() {
        ...
        if ( name.isEqualToString("") || surname.isEqualToString("") || username.isEqualToString("")  || password.isEqualToString("") || confirm_password.isEqualToString("") || email.isEqualToString("")) {
            ...
            var post:NSString = "name=\(name)&surname=\(surname)&email=\(email)&username=\(username)&password=\(password)"
    ...
    func isPasswordValid() -> Bool {
        ...
        if (passwordConfirmTxt.length >= 6 ){
            ...
            if(passwordTxt == passwordConfirmTxt){
                ...
                let decimalDigitSet = NSCharacterSet.decimalDigitCharacterSet()
                let a = passwordTxt.rangeOfCharacterFromSet(upperCaseSet).location
                if (isNSFound(passwordTxt.rangeOfCharacterFromSet(upperCaseSet).location) &&
                    isNSFound(passwordTxt.rangeOfCharacterFromSet(lowerCaseSet).location) &&
                    isNSFound(passwordTxt.rangeOfCharacterFromSet(decimalDigitSet).location)){
                    return true
                }
            }
        }
    ...
    func unValidPasswordError(){
        var alertView:UIAlertView = UIAlertView()
        alertView.title = "Sign Up Failed!"
        alertView.message = "Your password needs minimum 6 characters and" +
                            " at least one upper case character," +
                            " one lower case character and one numeric character"
        alertView.delegate = self
        alertView.addButtonWithTitle("OK")
        alertView.show()
    }
    ...
    func userError(){
        var alertView:UIAlertView = UIAlertView()
        alertView.title = "Sign Up Failed!"
        alertView.message = "There is an user which has same username with you." +
